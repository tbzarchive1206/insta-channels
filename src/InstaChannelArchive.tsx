...
export type Archive = { generatedAt: string; sourceFolderId: string; profiles: Collection[] };
...
function MediaActions({ media }: { media: Media }) {
  return <div className="image-actions"><span className="file-action-links"><a href={fileUrl(media.id)} target="_blank" rel="noreferrer">VIEW ↗</a><a href={directUrl(media.id)} target="_blank" rel="noreferrer">DOWNLOAD ↓</a></span></div>;
}

function MediaTile({ media }: { media: Media }) {
  if (media.kind === "video") return <figure className="media-tile video-tile"><a className="channel-video-link" href={fileUrl(media.id)} target="_blank" rel="noreferrer"><img src={thumbnail(media.id)} alt="" loading="lazy" /><span>VIDEO / OPEN IN GOOGLE DRIVE ↗</span></a><MediaActions media={media} /></figure>;
  if (media.kind === "audio") return <figure className="media-tile audio-tile"><div className="audio-mark" aria-hidden="true">AUDIO / INSTAGRAM CHANNEL</div><iframe src={previewUrl(media.id)} title="Instagram Channel audio" allow="autoplay" loading="lazy" /><MediaActions media={media} /></figure>;
  if (media.kind !== "image") return <figure className="media-tile unsupported-media"><a className="unsupported-tile" href={fileUrl(media.id)} target="_blank" rel="noreferrer">OPEN FILE IN DRIVE ↗</a><MediaActions media={media} /></figure>;
  return <figure className="media-tile"><a href={fileUrl(media.id)} target="_blank" rel="noreferrer" aria-label="Open original image in Google Drive"><img src={thumbnail(media.id)} alt="" loading="lazy" /></a><MediaActions media={media} /></figure>;
}

export function InstaChannelArchive({ data }: { data: Archive }) {
  const now = new Date();
  const [collection, setCollection] = useState<Collection | null>(null);
  const [year, setYear] = useState(now.getFullYear());
  const [month, setMonth] = useState(now.getMonth() + 1);
  const [shown, setShown] = useState(pageSize);
  const totalMedia = data.profiles.reduce((sum, item) => sum + item.media.length, 0);
  ...
  return <main id="top">
    <header className="masthead">
      <div className="utility"><a className="brand" href="https://tbzarchive1206.github.io/tbzarchive/">THE BOYZ / FAN ARCHIVE</a><nav><span>INSTAGRAM CHANNELS</span><span>/</span><a href="https://x.com/tbzarchive1206_" target="_blank" rel="noreferrer">TWITTER ↗</a></nav></div>
      <h1><span className="solid">INSTA CHANNEL</span><span className="outline">ARCHIVE</span></h1>
      <div className="stats"><p><strong>{data.profiles.length}</strong> PROFILES</p><i /><p><strong>{totalMedia.toLocaleString("en-US")}</strong> MEDIA FILES</p><i /><p>UPDATED <strong>{new Date(data.generatedAt).toLocaleDateString("en-GB")}</strong></p></div>
    </header>

    {!collection ? <section className="member-picker insta-picker channel-picker">
      ...
      <header className="member-gallery-head"><button onClick={() => setCollection(null)}>← ALL PROFILES</button><div><span>INSTAGRAM CHANNEL / PROFILE</span><h2>{collection.name.toUpperCase()}</h2></div><a href={folderUrl(collection.id)} target="_blank" rel="noreferrer">OPEN FOLDER ↗</a></header>
      ...
      {media.length ? <div className="media-grid">{media.slice(0, shown).map((item) => <MediaTile key={item.id} media={item} />)}</div> : <div className="empty member-empty"><strong>NO MEDIA</strong>THERE ARE NO UPLOADS FOR THIS MONTH.</div>}
